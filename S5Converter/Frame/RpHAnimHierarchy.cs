...
internal class RpHAnimHierarchy
{
    ...
    {
        [Flags]
        internal enum RpHAnimHierarchyFlag
        {
            None = 0,
            SubHierarchy = 0x01,
            NoMatrices = 0x02,
    ...
    internal static void RebuildNodeHierarchy(FrameWithExt[] frames)
    {
        ...
        static void BuildNodeArray(FrameWithExt[] frames, RpHAnimHierarchy hlist)
        {
            hlist.Nodes =
            [
                .. frames.Where(x => x.Extension.HanimPLG != null)
                    .Select(x => new Node() { NodeID = x.Extension.HanimPLG!.NodeID })
            ];
        ...
        static void RebuildNodes(FrameWithExt hierlist, RpHAnimHierarchy hlist, List<HInfo> hier)
        {
            ...
            {
                nodes[i] = h.N ?? throw new IOException($"hanim rebuild node {h.FrameIndex} missing node???");
                ++i;
