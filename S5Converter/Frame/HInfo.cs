...
internal class HInfo
{
    ...
    internal readonly List<HInfo> Children = [];
    ...
    {
        FrameWithExt? hierlist = frames.SingleOrDefault(x =>
            ((x.Extension.HanimPLG?.Nodes.Length ?? 0) > 0) || (x.Extension.HanimPLG?.ReBuildNodesArray ?? false));
    ...
    static void CheckEverythingHasFrame(RpHAnimHierarchy hlist, List<HInfo> hier)
    {
        ...
        {
            if (hier.All(x => x.NodeId != n.NodeID))
                throw new IOException($"hanim hierarchy info {n.NodeID} has no frame");
        }
    }

    private static List<HInfo> BuildHierarchy(FrameWithExt[] frames, RpHAnimHierarchy? hlist,
        Func<List<HInfo>, HInfo, HInfo?> getParent)
    {
    ...
    {
        (FrameWithExt? _, RpHAnimHierarchy? hlist) = GetHierarchy(frames);
        return BuildHierarchy(frames, hlist,
            (hier, i) => hier.FirstOrDefault(x => x.FrameIndex == i.F.Frame.ParentFrameIndex));
    }
    ...
    private static string Check(List<HInfo> hierA, List<HInfo> hierB, string nameA, string nameB)
    {
        string r = "";
        foreach (HInfo b in hierB)
        {
            HInfo? a = hierA.FirstOrDefault(x => x.FrameIndex == b.FrameIndex);
            if (a == null)
            {
                r += $"{b} is not in {nameA} hierarchy\n";
                continue;
            }
            ...
            {
                r +=
                    $"{b} parent existence missmatch {nameA}:{ToStringSafe(a.Parent)} <-> {nameB}:{ToStringSafe(b.Parent)}\n";
                continue;
            }
            ...
            {
                r += $"{b} parent missmatch {nameA}:{a.Parent} <-> {nameB}:{b.Parent}\n";
            }
        }
