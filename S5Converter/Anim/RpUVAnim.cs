...
internal class RpUVAnim : RtAnimAnimation, IDictEntry<RpUVAnim>
{
    ...
    internal new int Size => RtAnimAnimation.Size + sizeof(byte) * NameFixedStringSize +
                             sizeof(uint) * NodeToUVChannelMapSize + sizeof(int)
                             + (ParamKeyFrames?.Length ?? 0) * RpUVAnimParamKeyFrameData.Size +
                             (LinearKeyFrames?.Length ?? 0) * RpUVAnimLinearKeyFrameData.Size;
