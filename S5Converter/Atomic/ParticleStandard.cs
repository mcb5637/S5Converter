...
internal class RpPrtStdPropertyTable<TProperties> where TProperties : Enum
{
    internal int Id = 0;
    internal TProperties[] Ids = [];
    internal int[] Stride = [];
    ...
    internal static RpPrtStdPropertyTable<TProperties> Read(BinaryReader s, bool header)
    {
        if (header)
            ChunkHeader.FindChunk(s, RwCorePluginID.PRTSTDGLOBALDATA);
        RpPrtStdPropertyTable<TProperties> r = new()
        {
            Id = s.ReadInt32(),
        };
        int nProps = s.ReadInt32();
        r.Ids = new TProperties[nProps];
        for (int i = 0; i < nProps; ++i)
            r.Ids[i] = (TProperties)(object)s.ReadInt32();
...
internal class RpPrtStdEmitterStandard
{
    ...
    public int ParticleSizeSeriMisstake = 0;
    ...
    internal static RpPrtStdEmitterStandard Read(BinaryReader s, int flags, bool convertRad)
    {
        RpPrtStdEmitterStandard r = new()
        {
            ...
            ParticleSizeSeriMisstake = s.ReadInt32(),
            Color = RGBA.Read(s),
    ...
    internal void Write(BinaryWriter s, int flags, bool convertRad, uint versionNum, uint buildNum)
    {
        ...
        ParticleSize.Write(s);
        s.Write(ParticleSizeSeriMisstake);
        Color.Write(s);
        for (int i = 0; i < 4; ++i)
...
internal class RpPrtAdvEmtPointList
{
    ...
    internal int Size =>
        sizeof(int) * 5 + (PointList?.Length ?? 0) * Vec3.Size + (DirectionList?.Length ?? 0) * Vec3.Size;
