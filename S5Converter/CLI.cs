                    ...
                    foreach (RpAtomic a in d.Clp!.Atomics)
                    {
                        if (a.Extension.ParticleStandard != null)
                        {
                            foreach (RpPrtStdEmitter em in a.Extension.ParticleStandard.Emitters)
                            {
                                try
                                {
                                    using FileStream ou = new($"./emitters/{f.Name}_emitter_{emid}.json",
...
{
    public override unsafe float Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
    ...
    public override unsafe void Write(Utf8JsonWriter writer, float value, JsonSerializerOptions options)
    {
